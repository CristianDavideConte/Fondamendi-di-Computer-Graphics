...
#define NORMALS_TO_VERTEXES 0
#define NORMALS_TO_FACES 1
...
glm::vec3 wave_ambient = { 0.0, 0.0, 0.0 }, wave_diffuse = { 0.0, 0.0, 1.0 }, wave_specular{ 0.1, 0.3, 0.5 }; GLfloat wave_shininess = 50.0;
glm::vec3 custom_material_ambient = { 0.1, 0.3, 0.5 }, custom_material_diffuse = { 0.3, 0.3, 1.0 }, custom_material_specular = { 0.1, 0.3, 0.5 }; GLfloat custom_material_shininess = 50.0;
...
typedef enum {
	...
	CHANGE_TO_WCS,
	CHANGE_TO_OCS,
	NORMALS_TO_FACES_MODE,
	NORMALS_TO_VERTEXES_MODE
} MenuOption;
...
int normals_mode = NORMALS_TO_VERTEXES;
...
void init_waving_plane() {
	...
	Object obj4 = {};
	obj4.mesh = sphereS;
	obj4.material = MaterialType::WAVE_MATERIAL;
...
void init_mesh() {
	...
	Object obj4 = {};
	obj4.mesh = sphereS;
	obj4.material = MaterialType::CUSTOM_MATERIAL; // NO_MATERIAL;
...
void initShader()
{
	...
	//Wave Shader Loading
	shaders_IDs[WAVE] = createProgram(ShaderDir + "v_wave.glsl", ShaderDir + "f_blinn.glsl");
	//Otteniamo i puntatori alle variabili uniform per poterle utilizzare in seguito
	...
	base_unif.time_delta_pointer = glGetUniformLocation(shaders_IDs[WAVE], "current_time");
...
void init() {
	...
	// Materials setup
	materials.resize(7);
	materials[MaterialType::CUSTOM_MATERIAL].name = "Custom material";
	materials[MaterialType::CUSTOM_MATERIAL].ambient = custom_material_ambient;
	materials[MaterialType::CUSTOM_MATERIAL].diffuse = custom_material_diffuse;
	materials[MaterialType::CUSTOM_MATERIAL].specular = custom_material_specular;
	materials[MaterialType::CUSTOM_MATERIAL].shininess = custom_material_shininess;

	materials[MaterialType::WAVE_MATERIAL].name = "Wave material";
	materials[MaterialType::WAVE_MATERIAL].ambient = wave_ambient;
	materials[MaterialType::WAVE_MATERIAL].diffuse = wave_diffuse;
	materials[MaterialType::WAVE_MATERIAL].specular = wave_specular;
	materials[MaterialType::WAVE_MATERIAL].shininess = wave_shininess;
...
void main_menu_func(int option)
{
	...
	case MenuOption::CHANGE_TO_WCS: TransformMode = WCS;
		break;
	case MenuOption::NORMALS_TO_FACES_MODE: 
		normals_mode = NORMALS_TO_FACES;

		for (int i = 0; i < objects.size(); i++) {
			glDeleteVertexArrays(1, &objects[i].mesh.vertexArrayObjID);
		}
		...
		//initShader();
		break;
	case MenuOption::NORMALS_TO_VERTEXES_MODE:
		normals_mode = NORMALS_TO_VERTEXES;
...
void buildOpenGLMenu()
{
	...
	glutAddMenuEntry(materials[MaterialType::CUSTOM_MATERIAL].name.c_str(), MaterialType::CUSTOM_MATERIAL);
	glutAddMenuEntry(materials[MaterialType::WAVE_MATERIAL].name.c_str(), MaterialType::WAVE_MATERIAL);
	...
	glutAddMenuEntry("Normals to vertexes", MenuOption::NORMALS_TO_VERTEXES_MODE);
	glutAddMenuEntry("Normals to faces", MenuOption::NORMALS_TO_FACES_MODE);
...
{
	if (generate) {
		// Faccio l'unbind dei vao precedenti, se assegnati
		glDeleteVertexArrays(1, &mesh->vertexArrayObjID);
...
void loadObjFile(string file_path, Mesh* mesh)
{
	...
	if (tmp_normals.size() == 0) {
		...
		for (int i = 0; i < vertexIndices.size(); i += 3)
		{
			...
			if (normals_mode == NORMALS_TO_VERTEXES) {
				//Normali ai vertici
			...
			} else if (normals_mode == NORMALS_TO_FACES) {
				//Normali alle facce
...
void printToScreen()
{
	...
	normals += normals_mode == NORMALS_TO_FACES ? "faces" : "vertexes";
...
int main(int argc, char** argv) {
	...
	glutCreateWindow("Navigazione iterattiva in scena con modelli mesh 3D - Cristian Davide Conte");
