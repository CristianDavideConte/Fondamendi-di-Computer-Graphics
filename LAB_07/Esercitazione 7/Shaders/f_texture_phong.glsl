...
in vec3 L;
in vec3 E;
in vec2 _TexCoord; //Prendo in input la texture
...
void main() {    
    ...
    vec3 diffuse = light.power * light.color * diff * texture(textureBuffer,_TexCoord).xyz; //La componente diffuse è data dalla texture e non dal materiale
