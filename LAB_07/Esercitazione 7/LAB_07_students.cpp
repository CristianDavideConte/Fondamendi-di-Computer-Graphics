...
#define USE_TORUS_PROCEDURAL_TEXTURE true
#define USE_TORUS_CHECKERBOARD_TEXTURE true
...
constexpr float CAMERA_TRASLATION_SPEED_X = 0.2f;
constexpr float CAMERA_TRASLATION_SPEED_Y = 0.01f;
...
std::vector< glm::vec4> path; //camera path
...
struct further_from_camera {
	...
	{
		const float dist1 = glm::length(obj1.M * glm::vec4(0.f, 0.f, 0.f, 1.f) - ViewSetup.position);
		const float dist2 = glm::length(obj2.M * glm::vec4(0.f, 0.f, 0.f, 1.f) - ViewSetup.position);
		return dist1 > dist2;
	}
};

vector<Object> sortByDistance(vector<Object> objs) {
	vector<Object> orderedObjs{};
	for (int i = 0; i < objs.size(); i++)
		orderedObjs.push_back(objs[i]);

	std::sort(orderedObjs.begin(), orderedObjs.end(), further_from_camera());
	return orderedObjs;
}
...
// From LAB_01
float* deCasteljau(float t, float iterArray[MaxNumPts + 1][3], int numPoints) {
...
void computeCameraPath() {	
	...
	for (float k = 0; k <= MaxNumPtsCurve; k++) {
		...
		float temp[MaxNumPts + 1][3]{};
		for (int i = 0; i < dim; i++) {
			temp[i][0] = keyframes.at(i)[0];
			temp[i][1] = keyframes.at(i)[1];
			temp[i][2] = keyframes.at(i)[2];
		}

		//Back to the first point
		temp[dim][0] = keyframes.at(0)[0];
		temp[dim][1] = keyframes.at(0)[1];
...
void init_reflective_sphere() {
	...
	obj.material = MaterialType::NO_MATERIAL;
	obj.shading = ShadingType::REFLECTION; // PASS_THROUGH; //TEXTURE_ONLY;
...
void init_refractive_obj() {
	...
	obj.diffuseTexID = cubeTexture;
	obj.material = MaterialType::NO_MATERIAL;
	obj.shading = ShadingType::REFRACTION; //TEXTURE_ONLY; // PASS_THROUGH;  
	obj.name = "Glass Cube";
...
void init_textured_plane() {
	...
	Mesh surface = {};
	surface.vertices = { {-2,0,-2}, {-2,0,2}, {2,0,2}, {2,0,-2}, {-2,0,-2}, {2,0,2} }; //Due triangoli
	...
	//  Dentro image[64][64] c'è un 2x2 della scacchiera che poi viene ripetuta.
	/////////////////////////////////////////////////////////////////////////
	...
	glTexImage2D(GL_TEXTURE_2D,  //the target
		...
		image); //Al posto di passare una texture passo la mia image (texture generata dinamicamente)
	obj0.name = "Plane";
	obj0.M = glm::translate(glm::mat4(1), glm::vec3(0., 0., -5.));
	objects.push_back(obj0);
}

void load_procedural_texture_torus() {
	Mesh surface = {};
	compute_Torus(&surface);
	generate_and_load_buffers(true, &surface);
	Object obj = {};
	obj.mesh = surface;
	...
	if (USE_TORUS_CHECKERBOARD_TEXTURE) { 
	...
	glTexImage2D(GL_TEXTURE_2D,  //the target
		0, // the mip map level we want to generate
		GL_RGB, // the format of the texture
		texture_size, //texture_size, width
		texture_size, //texture_size, heigth
		0,  // border, leave 0
		GL_RGB, // we assume is a RGB color image with 24 bit depth per pixel
		GL_UNSIGNED_BYTE, // the data type
		image
...
void init_rock() {
	...
	obj.material = MaterialType::SLATE;
	obj.shading = ShadingType::NORMAL_MAPPING; //TEXTURE_ONLY;
...
void init_brick_column() {
	...
	objects.push_back(obj);
	movables.push_back(objects.size() - 1);
	obj.shading = ShadingType::TEXTURE_ONLY; // TEXTURE_PHONG; // NORMAL_MAPPING;
...
void init_skybox() {
	...
	obj.name = "skybox";
	obj.M = glm::scale(glm::translate(glm::mat4(1), glm::vec3(0, 0, 0)), glm::vec3(1., 1., 1.));
	objects.push_back(obj);
}
...
void init_torus() {
	if (USE_TORUS_PROCEDURAL_TEXTURE) {
		load_procedural_texture_torus();
		return;
	}
	...
	obj.shading = ShadingType::TEXTURE_PHONG;  // PASS_THROUGH; // TEXTURE_ONLY;  
	obj.name = "Torus";
...
void initShader()
{
	...
	glUniform1i(glGetUniformLocation(shaders_IDs[REFLECTION], "skybox"), 0);
	...
	glUniform1i(glGetUniformLocation(shaders_IDs[REFRACTION], "skybox"), 0);
...
void drawScene() {
	...
	vector<Object> transparentObjs;

	for (int i = 0; i < objects.size(); i++) {
		// If you handle transparency then you must jump any semi transparent object
		const bool hasTransparency = objects.at(i).blended;
		if (!hasTransparency) {
			//Shader selection
			...
			transparentObjs.push_back(objects.at(i));
		}
	}

	// Transparent objects reordering
	// Draw in a reversed order: from farest to nearest
	transparentObjs = sortByDistance(transparentObjs);
	glDepthMask(GL_FALSE);
	for (int i = 0; i < transparentObjs.size(); i++) {
		//Shader selection
		switch (transparentObjs[i].shading) {
		case ShadingType::NORMAL_MAPPING:
			glUseProgram(shaders_IDs[NORMAL_MAPPING]);
			// Caricamento matrice trasformazione del modello
			glUniformMatrix4fv(base_uniforms[NORMAL_MAPPING].M_Matrix_pointer, 1, GL_FALSE, value_ptr(transparentObjs[i].M));
			//Material loading
			glUniform3fv(light_uniforms[NORMAL_MAPPING].material_ambient, 1, glm::value_ptr(materials[transparentObjs[i].material].ambient));
			glUniform3fv(light_uniforms[NORMAL_MAPPING].material_diffuse, 1, glm::value_ptr(materials[transparentObjs[i].material].diffuse));
			glUniform3fv(light_uniforms[NORMAL_MAPPING].material_specular, 1, glm::value_ptr(materials[transparentObjs[i].material].specular));
			glUniform1f(light_uniforms[NORMAL_MAPPING].material_shininess, materials[transparentObjs[i].material].shininess);
			glActiveTexture(GL_TEXTURE0);
			glBindTexture(GL_TEXTURE_2D, transparentObjs[i].diffuseTexID);
			glActiveTexture(GL_TEXTURE1);
			glBindTexture(GL_TEXTURE_2D, transparentObjs[i].normalTexID);
			break;
		case ShadingType::TEXTURE_PHONG:
			glUseProgram(shaders_IDs[TEXTURE_PHONG]);
			// Caricamento matrice trasformazione del modello
			glUniformMatrix4fv(base_uniforms[TEXTURE_PHONG].M_Matrix_pointer, 1, GL_FALSE, value_ptr(objects[i].M));
			//Material loading
			glUniform3fv(light_uniforms[TEXTURE_PHONG].material_ambient, 1, glm::value_ptr(materials[transparentObjs[i].material].ambient));
			glUniform3fv(light_uniforms[TEXTURE_PHONG].material_diffuse, 1, glm::value_ptr(materials[transparentObjs[i].material].diffuse));
			glUniform3fv(light_uniforms[TEXTURE_PHONG].material_specular, 1, glm::value_ptr(materials[transparentObjs[i].material].specular));
			glUniform1f(light_uniforms[TEXTURE_PHONG].material_shininess, materials[transparentObjs[i].material].shininess);
			glActiveTexture(GL_TEXTURE0);
			glBindTexture(GL_TEXTURE_2D, transparentObjs[i].diffuseTexID);
			break;
		case ShadingType::SKYBOX:
			glUseProgram(shaders_IDs[SKYBOX]);
			// Caricamento matrice trasformazione del modello
			glUniformMatrix4fv(base_uniforms[SKYBOX].M_Matrix_pointer, 1, GL_FALSE, value_ptr(transparentObjs[i].M));
			glActiveTexture(GL_TEXTURE0);
			glBindTexture(GL_TEXTURE_CUBE_MAP, transparentObjs[i].diffuseTexID);
			// We draw the skybox first, so if we disable writes on the Z-BUFFER, 
			// all later draw calls will overwrite the skybox pixels for sure, 
			// no matter how distant is the skybox.
			glDepthMask(GL_FALSE);
		...
		case ShadingType::REFLECTION:
			glUseProgram(shaders_IDs[REFLECTION]);
			glUniformMatrix4fv(base_uniforms[REFLECTION].M_Matrix_pointer, 1, GL_FALSE, value_ptr(transparentObjs[i].M));
			glActiveTexture(GL_TEXTURE0);
			glBindTexture(GL_TEXTURE_CUBE_MAP, transparentObjs[i].diffuseTexID);
			break;
		case ShadingType::REFRACTION:
			glUseProgram(shaders_IDs[REFRACTION]);
			glUniformMatrix4fv(base_uniforms[REFRACTION].M_Matrix_pointer, 1, GL_FALSE, value_ptr(transparentObjs[i].M));
			glActiveTexture(GL_TEXTURE0);
			glBindTexture(GL_TEXTURE_CUBE_MAP, transparentObjs[i].diffuseTexID);
			break;
		case ShadingType::TEXTURE_ONLY:
			glUseProgram(shaders_IDs[TEXTURE_ONLY]);
			// Caricamento matrice trasformazione del modello
			glUniformMatrix4fv(base_uniforms[TEXTURE_ONLY].M_Matrix_pointer, 1, GL_FALSE, value_ptr(transparentObjs[i].M));
			glActiveTexture(GL_TEXTURE0);
			glBindTexture(GL_TEXTURE_2D, transparentObjs[i].diffuseTexID);
			break;
		case ShadingType::PASS_THROUGH:
			glUseProgram(shaders_IDs[PASS_THROUGH]);
			// Caricamento matrice trasformazione del modello
			glUniformMatrix4fv(base_uniforms[PASS_THROUGH].M_Matrix_pointer, 1, GL_FALSE, value_ptr(transparentObjs[i].M));
			break;
		default:
			break;
		}
		...
		glBindVertexArray(transparentObjs[i].mesh.vertexArrayObjID);
		glDrawArrays(GL_TRIANGLES, 0, transparentObjs[i].mesh.vertices.size());
...
void refresh_monitor(int millis)
{
	if (isAnimationPlaying) {
		currentKeyframe = currentKeyframe >= path.size() - 1 ? 0 : currentKeyframe + 1;
		ViewSetup.position = path.at(currentKeyframe);
	}
...
void keyboardDown(unsigned char key, int x, int y)
{
	...
	case 'n':
		...
		break;
	case ' '://Play/Pause camera path animation
...
void moveCameraUp()
{
	glm::vec3 direction = ViewSetup.target - ViewSetup.position;
	glm::vec3 slide_vector = glm::normalize(glm::cross(direction, glm::vec3(ViewSetup.upVector)));
	glm::vec3 upDirection = glm::cross(direction, slide_vector) * CAMERA_TRASLATION_SPEED_Y;
...
void moveCameraDown()
{
	...
	glm::vec3 upDirection = glm::cross(glm::vec3(direction), slide_vector) * CAMERA_TRASLATION_SPEED_Y;
	ViewSetup.position += glm::vec4(upDirection, 0.0);
	ViewSetup.target += glm::vec4(upDirection, 0.0);
}
...
void modifyModelMatrix(glm::vec3 translation_vector, glm::vec3 rotation_vector, GLfloat angle, GLfloat scale_factor)
{
	...
	glm::vec3 scaling_vector = glm::vec3(1.0f);

	switch (WorkingAxis) {
	case X: scaling_vector.x *= scale_factor; break;
	case Y: scaling_vector.y *= scale_factor; break;
	case Z: scaling_vector.z *= scale_factor; break;
	}
...
//Ogni volta che voglio caricare un'immagine diversa devo chiamare loadTexture.
//loatTexture segue gli steps del pdf OpenlGL 3.
GLuint loadTexture(string path)
{
...
int main(int argc, char** argv) {
	...
	glutCreateWindow("Model Viewer Texture|Normal|Env Mapping - Cristian Davide Conte");
